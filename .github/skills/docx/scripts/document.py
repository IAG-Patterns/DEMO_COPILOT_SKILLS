...
def _ns(uri: str) -> str:
    """Return an XML namespace / relationship type URI.

    Many OOXML identifiers are defined as `http`-scheme URIs (identifiers, not fetched).
    Some scanners flag literal scheme+separator strings, so build them without embedding
...
class DocxXMLEditor(XMLEditor):
    ...
    def _ensure_w16du_namespace(self):
        ...
        if not root.hasAttribute("xmlns:w16du"):  # type: ignore
            root.setAttribute(  # type: ignore
                "xmlns:w16du",
                _ns("schemas.microsoft.com/office/word/2023/wordml/word16du"),
            )
    ...
    def _ensure_w16cex_namespace(self):
        ...
        if not root.hasAttribute("xmlns:w16cex"):  # type: ignore
            root.setAttribute(  # type: ignore
                "xmlns:w16cex",
                _ns("schemas.microsoft.com/office/word/2018/wordml/cex"),
            )
    ...
    def _ensure_w14_namespace(self):
        ...
        if not root.hasAttribute("xmlns:w14"):  # type: ignore
            root.setAttribute(  # type: ignore
                "xmlns:w14",
                _ns("schemas.microsoft.com/office/word/2010/wordml"),
            )
    ...
    def suggest_paragraph(xml_content: str) -> str:
        ...
        """
        wrapper = (
            f'<root xmlns:w="{_ns("schemas.openxmlformats.org/wordprocessingml/2006/main")}">'
            f"{xml_content}</root>"
        )
...
def _generate_hex_id() -> str:
    ...
    """
    return f"{(secrets.randbelow(0x7FFFFFFE) + 1):08X}"
...
class Document:
    ...
    def _add_relationship_for_people(self, path):
        ...
        rel_xml = (
            ...
            f'Type="{_ns("schemas.microsoft.com/office/2011/relationships/people")}" '
            'Target="people.xml"/>'
        )
  ...
  <w15:presenceInfo w15:providerId="None" w15:userId="{escaped_author}"/>
    ...
    def _ensure_comment_relationships(self):
        ...
        rels = [
            (
                next_rid_num,
                _ns("schemas.openxmlformats.org/officeDocument/2006/relationships/comments"),
                "comments.xml",
            ),
            (
                next_rid_num + 1,
                _ns("schemas.microsoft.com/office/2011/relationships/commentsExtended"),
                "commentsExtended.xml",
            ),
            (
                next_rid_num + 2,
                _ns("schemas.microsoft.com/office/2016/09/relationships/commentsIds"),
                "commentsIds.xml",
            ),
            (
                next_rid_num + 3,
                _ns("schemas.microsoft.com/office/2018/08/relationships/commentsExtensible"),
