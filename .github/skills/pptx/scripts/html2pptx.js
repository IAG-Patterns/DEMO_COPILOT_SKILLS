...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const parseBoxShadow = (boxShadow) => {
      ...
      // Safety: avoid pathological inputs (regex DoS / extremely long strings)
      if (typeof boxShadow === 'string' && boxShadow.length > 2000) return null;
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    // Use a private temp dir (avoid predictable / public temp paths)
    const runTmpDir = await fs.promises.mkdtemp(path.join(tmpDir, 'html2pptx-'));

    try {
      // Use Chrome on macOS, default Chromium on Unix
      const launchOptions = { env: { TMPDIR: runTmpDir } };
      ...
      await fs.promises.rm(runTmpDir, { recursive: true, force: true });
    }
  } catch (error) {
